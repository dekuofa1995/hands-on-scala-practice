...
class SudokuResolverSpec extends AnyWordSpec with Matchers {
  import SudokuResolver._
  ...
  "resolver" should {
    "test" in {
      ...
      val result = resolve(puzzle)
      result.foreach(_.foreach(line => println(line.mkString(" "))))
    }
  }
